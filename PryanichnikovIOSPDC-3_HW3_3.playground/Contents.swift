...
// Решение задачи №1:
var arrayOfLessons: [String] = []


func addLesson (lesson: String, lessons: [String]){
    arrayOfLessons.append(lesson)
    arrayOfLessons += lessons
}

/* Если попытаться удалить значение которого не существует в массиве, то произойдет ошибка. Нужно сделать проверку на наличие значение в массиве и производить удаление только в случае наличия значения.*/
func deleteLesson (lesson: String, lessons: [String]){
    if arrayOfLessons.isEmpty == false{
        if let index = arrayOfLessons.firstIndex(of: lesson){
            arrayOfLessons.remove(at: index)
        }
        else{
            print("Элемент \(lesson) отсутствует в хранилище, удаление невозможно")
        }
        for number in lessons{
            if let index = arrayOfLessons.firstIndex(of: number){
                arrayOfLessons.remove(at: index)
            }
            else{
...
addLesson(lesson: "Biology", lessons: ["History", "Maths"])
deleteLesson(lesson: "Biology", lessons: ["History", "Russian Language"])
print(arrayOfLessons)
...
func convertBonusToMoney (from: [Int: Double]) -> [Int: Int]{
    for (clientId, clientBonus) in userPoints{
        userMoney.updateValue(Int(clientBonus / 100 * 10), forKey: clientId)
    }
    return userMoney
}

convertBonusToMoney(from: userPoints)
print(userMoney)
